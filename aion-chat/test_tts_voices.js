...
const voices = [
  ...
  { uri: 'minimax:CloneVoice001', customName: 'MiniMax · 配置中的 AI' },
...
test('chat/theater picker keeps either saved route and labels both', () => {
  ...
  assert.deepEqual(select.children.map(n => n.label), ['硅基流动', 'MiniMax', 'Edge 免费']);
  assert.equal(select.options[1].textContent, 'MiniMax · 配置中的 AI');
  assert.equal(select.options[2].textContent, 'Edge 免费 · 晓晓 · 女声');
});

test('provider outage does not silently switch the saved route', () => {
  const select = selectElement();
  assert.equal(populate(select, [voices[2]], voices[0].uri), voices[0].uri);
  assert.equal(select.options.length, 2);
  assert.equal(populate(select, [voices[2]], ''), voices[2].uri);
});

test('removing a free favorite clears the selection without choosing a paid voice', () => {
  const select = selectElement();
  assert.equal(populate(select, [voices[0]], voices[2].uri), '');
  assert.equal(select.options.length, 2);
});
