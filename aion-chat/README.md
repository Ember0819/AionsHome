...
**小说与对话共用小剧场界面（2026-09）** — 每个故事可切换对话/小说模式，旧对话仍保留。小说先与所选角色聊天讨论脑洞，再点“按讨论生成大纲”；生成故事共识和章节计划后，必须明确确认大纲才能开始写正文（默认目标 6500 字）。不满意可继续讨论、重新生成或恢复最近三版大纲；中途“商量剧情”后重新确认，不自动覆盖已有正文。目录支持选章阅读与收听。讨论、拟纲、写作均带所选小剧场角色的人设，没有完整人设时提示选择。讨论原文单独保存，超过12条尚未摘要的非空消息时整理早期讨论并保留最近8条用于后续讨论；正文请求只带确认后的故事共识，不带讨论原文。正文自动保存草稿，重写保留最近三版，改动前文后提示检查后续章节衔接。故事摘要只取已写正文，与日常聊天记忆隔离。章节上下文详见 [小说上下文说明](../docs/theater-novel-context.md)。

**手动开启流式朗读** — 正文生成期间即可点“开始朗读”：补送已有正文后跟随新增文字，每段约 300–500 字，不提前生成下一章语音。播放器支持暂停、在已就绪的连续片段内拖动、独立停止合成和失败续试。切换阅读章节不打断播放；当前设备保存阅读与收听位置。对话模式也改为手动开启语音，并复用已有整段语音缓存。
